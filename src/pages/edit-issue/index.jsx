...
import MediaUpload from '../report-issue/components/MediaUpload'; 
import ReportSummary from '../report-issue/components/ReportSummary'; 
import { useAuth } from '../../context/AuthContext';

const EditIssue = ({ onEditSuccess, hideHeader = false, editingComplaint }) => {
  const { user } = useAuth();
  const isEditing = !!editingComplaint;
  ...
  const handleSubmit = async () => {
    ...
    if (!isEditing && formData.media.length > 0 && formData.media[0].file) {
      data.append('image', formData.media[0].file);
    }
